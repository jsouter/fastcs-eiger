...
async def test_introspection(sim_eiger_controller: EigerController):
    ...
    for subsystem_name, subcontroller in controller.get_sub_controllers().items():
        subsystem = subcontroller.subsystem
        serialised_parameters[subsystem_name] = {}
        subsystem_parameters[subsystem_name] = await subsystem.introspect(
    ...
    detector_attributes = _create_attributes(subsystem_parameters["DETECTOR"])
    assert len(detector_attributes) == 76
    monitor_attributes = _create_attributes(subsystem_parameters["MONITOR"])
    assert len(monitor_attributes) == 7
    stream_attributes = _create_attributes(subsystem_parameters["STREAM"])
    ...
    assert detector_attributes["humidity"]._group == "Status"
